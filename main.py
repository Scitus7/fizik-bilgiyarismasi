...
pygame.display.set_caption("efsanevi fizik projesi")
DOT_SIZE = 50
new_width = DOT_SIZE
new_height = DOT_SIZE
Ghost_number = 3
background = random.choice(["background","renkli","kırmızılı"])
background_image = pygame.image.load(f"{background}.png").convert()
# Load game assets

karakterler = ["einstein","newton","Leonardo Da Vinci","aristotales","Kübra Hoca","İbn-i Heysem"]
s = 0
...
gamestart_imageo = pygame.image.load("gamestart.png")
...
planets = ["planet","moon","island","karadelik","lava","yıldız"]
...
while giriş:
    ...
    intro_image.set_alpha(alpha_value)
    pygame.display.flip()
    time.sleep(0.3)
    if alpha_value < 3:
        giriş = False
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_q:
                print("q")
                subprocess.call(['python', 'soru.py'])
                durdur = 0
